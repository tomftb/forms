...
class Form{
    ...
    setUpParameters(){
        console.log('Forms.setUpParameters()'); 
        /* TO DO -> EXTEND FOR ALL GLOSSARY */
        this.Xhr.run(this.getXhrParm('GET','loadFormsDefault','setUpDefaults'));
    }
    setUpDefaults(response){
        console.log('Forms.setUpDefaults()');
    ...
    setTableResponse(response){
        console.log('Forms.setTableResponse()');
        //console.log(response);
        try {
 ...
 */
try{
    var MyForm = new Form(window.appUrl,window.appUrl+'/router.php?task=');
}
catch (e){
    console.log(e);
    error=true;
...
if(error===false){
    ...
    try{
        MyForm.Modal.setLink();
        MyForm.Table.setLink();
        /* SETUP PARAMETERS => Glossary */
        MyForm.setUpParameters();
    }
    catch (error){
        console.log(error);
