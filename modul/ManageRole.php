...
class ManageRole
{
    ...
    private function updateRole($sql)
    {
         ...
         //print_r(array_merge($sql,$sql_upd));
        try{
            $this->dbLink->beginTransaction(); //PHP 5.1 and new
            $this->dbLink->query('UPDATE `slo_rola` SET `NAZWA`=:n ,`mod_user_id`=:mod_user_id,`mod_user_login`=:mod_user_login,`mod_user_full_name`=:mod_user_full_name,`mod_user_email`=:mod_user_email,`mod_host`=:mod_host WHERE `ID`=:i',array_merge($sql,$sql_upd));
    ...
    private function checkPermRolePairExitsts($sql){
        $this->Log->log(0,"[".__METHOD__."]");
        if(!is_array($this->dbLink->squery('SELECT * FROM `upr_i_slo_rola` WHERE id_rola=:idr AND id_upr=:idu',$sql,'FETCH_ASSOC','fetch'))){
            return false;
        }
        return true;
    }
    private function checkExistSloPerm($idPerm){
        ...
        if(!is_array($this->dbLink->squery('SELECT * FROM `uprawnienia` WHERE ID=:i',[':i'=>[$idPerm,'INT']],'FETCH_ASSOC','fetch'))){
