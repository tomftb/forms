...
class ManagePermission
{
    ...
    public function uPermUsers(){
        ...
        try{
            ...
            array_map(array($this, 'deleteUserFromPerm'),$this->dbLink->squery('SELECT uu.`id_uzytkownik` as `idu` FROM `uzyt_i_upr` uu, `uzytkownik` u WHERE uu.`id_uzytkownik`=u.`id` AND u.`wsk_u`="0" AND `id_uprawnienie`=:i',[':i'=>[$this->idPerm,'INT']],'FETCH_ASSOC','fetchAll'));
            /* ADD SENDED USERS */
            self::addUserToPerm(); 
            $this->dbLink->commit();  
        }
    ...
    private function checkUserExist($idUser=0){
        $this->Log->log(0,"[".__METHOD__."]");
        $userData=$this->dbLink->squery('SELECT `id`,`wsk_u` FROM `uzytkownik` WHERE `id`=:i',[':i'=>[$idUser,'INT']],'FETCH_ASSOC','fetch');
    ...
    public function getAllPerm()
    {
        ...
        $sql=[':f'=>[$f,'STR']];
        $result['perms']=$this->dbLink->squery('SELECT `ID` as \'i\',`SKROT` as \'s\',`NAZWA` as \'n\',`OPIS` as \'o\' FROM `uprawnienia` WHERE ID LIKE (:f) OR NAZWA LIKE (:f) OR SKROT LIKE (:f) OR OPIS LIKE (:f) ORDER BY ID asc',$sql,'FETCH_ASSOC','fetchAll');
