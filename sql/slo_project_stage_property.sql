...
--
-- Indeksy dla tabeli `slo_project_stage_property`
--
ALTER TABLE `slo_project_stage_property`
  ADD KEY `FK_SloStagePropertyCreateUser` (`create_user_id`),
  ADD KEY `FK_SloStagePropertyModifyUser` (`mod_user_id`),
  ADD KEY `FK_SloStagePropertyParent` (`id_parent`);
...
--
-- Constraints for table `slo_project_stage_property`
--
ALTER TABLE `slo_project_stage_property`
  ADD CONSTRAINT `FK_SloStagePropertyCreateUser` FOREIGN KEY (`create_user_id`) REFERENCES `uzytkownik` (`id`),
  ADD CONSTRAINT `FK_SloStagePropertyModifyUser` FOREIGN KEY (`mod_user_id`) REFERENCES `uzytkownik` (`id`),
  ADD CONSTRAINT `FK_SloStagePropertyParent` FOREIGN KEY (`id_parent`) REFERENCES `slo_project_stage` (`id`);
COMMIT;

ALTER TABLE `slo_project_stage_property`
ADD CONSTRAINT `UNIQUE_StageProperty` UNIQUE KEY(`id_parent`,`property`);
COMMIT;
